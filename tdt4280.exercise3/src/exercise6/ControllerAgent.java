...
public class ControllerAgent extends Agent {
    ...
    protected void setup() {
        ...
        addBehaviour(new SuperBehaviour(this));
...
class SuperBehaviour extends CyclicBehaviour {
    ...
    public void action() {
        ...
        if (msg != null) {
                ...
                for(double[] agentVals : myAgent.agentInfo.values()){
                    if(bearing < 180.0){
                        ...
                        if(bearing > 90.0 && bearing < 270.0){
                            //Northwest
                            if(agentVals[0] > position[0] && agentVals[1] > position[1]) closerFriend = true;
                        }else{
                            //Southwest
                            if(agentVals[0] > position[0] && agentVals[1] < position[1]) closerFriend = true;
                        }
            ...
            if (turnRight) {
                if(distance_delta == 0.0) motor_speeds[0] = -25.0;
                else motor_speeds[0] = -35.0;
                motor_speeds[1] = 35.0;
            } else {
                motor_speeds[0] = 35.0;
                if(distance_delta == 0.0) motor_speeds[1] = -25.0;
                else motor_speeds[1] = -35.0;
            }
            ACLMessage reply = msg.createReply();
            reply.setContent(motor_speeds[0] + ", " + motor_speeds[1]);
            myAgent.send(reply);
